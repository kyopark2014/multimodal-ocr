...
Arguments:
    ...
    output_dir  Optional. Directory where page images will be saved
                (created automatically if it does not exist).
                If omitted, images are written to
                <repo_root>/artifacts/<pdf_basename_without_ext>/
...
def _default_output_dir(pdf_path: str) -> str:
    """`<repo_root>/artifacts/<pdf_stem>/` — repo root is parent of pdf2img/."""
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    stem = os.path.splitext(os.path.basename(os.path.expanduser(pdf_path)))[0]
    return os.path.join(repo_root, "artifacts", stem)
...
def main():
    if len(sys.argv) < 2:
        ...
        print(
            "  output_dir defaults to <repo>/artifacts/<pdf_basename_without_ext>/",
            file=sys.stderr,
        )
        sys.exit(1)

    pdf_path = sys.argv[1]
    output_dir = sys.argv[2] if len(sys.argv) >= 3 else _default_output_dir(pdf_path)
